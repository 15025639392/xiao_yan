...
// 输入 PNG 文件路径
const inputPath = path.join(__dirname, './src-tauri/icons/logo.png')
// 临时输出带圆角和 padding 的 PNG 文件路径
const processedOutputPath = path.join(__dirname, './src-tauri/processed-logo.png')
// 输出 ICNS 文件路径
...
sharp(inputPath)
    ...
    .composite([
        // rx ry是圆角半径
        {
            input: Buffer.from(
                `<svg>
         <rect x="0" y="0" width="784" height="784" rx="250" ry="250" />
       </svg>`
            ),
            blend: 'dest-in',
        },
