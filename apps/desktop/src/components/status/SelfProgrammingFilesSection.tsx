import type { SelfProgrammingJobState } from "./selfProgrammingTypes";

type SelfProgrammingFilesSectionProps = {
  touchedFiles: SelfProgrammingJobState["touched_files"];
};

export function SelfProgrammingFilesSection({ touchedFiles }: SelfProgrammingFilesSectionProps) {
  if (!touchedFiles || touchedFiles.length === 0) {
    return null;
  }

  return (
    <div className="si-section">
      <h4 className="si-section__title">触碰文件</h4>
      <div className="si-file-list">
        {touchedFiles.map((filePath) => (
          <span key={filePath} className="si-file-tag">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-8V8z" />
              <polyline points="14 2 14 8 20 8" />
            </svg>
            {filePath}
          </span>
        ))}
      </div>
    </div>
  );
}
