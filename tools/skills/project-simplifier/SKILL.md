...
项目初期使用本 skill 时，不要默认把“简化”理解成“全仓治理”。
...
只有用户明确要求全局收敛，或当前问题已经跨多个主入口时，才进入完整扫描模式。
...
1. 明确当前最小目标
2. 只跑和目标相关的 1 到 2 个脚本
3. 只分析目标路由、目标页面或目标模块
4. 只产出当前范围内的保留 / 延后 / 删除候选

适合示例：

- 只分析某个桌面页面是否还能留在默认主链路
- 只判断某组 API 是否还能保留
- 只收敛某个边缘能力，不碰其他域
...
如果只是收敛单路由、单页面或单模块，优先从下面顺序里选最少的命令，而不是默认全跑：
