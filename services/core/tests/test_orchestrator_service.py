...
def test_scheduler_tick_dispatches_parallel_tasks_to_multiple_engineers_in_one_session(tmp_path: Path) -> None:
    project = tmp_path / "demo-tauri-project"
    (project / "src").mkdir(parents=True)
    (project / "src-tauri" / "src").mkdir(parents=True)
    (project / "src" / "main.ts").write_text("console.log('frontend')\n", encoding="utf-8")
    ...
    service = OrchestratorService(
        ...
        state_store=state_store,
        verification_runner=lambda _path, commands: OrchestratorVerification(commands=commands, passed=True),
        max_parallel_tasks_per_session=2,
    )
    ...
    running_session = service.get_session(session.session_id)

    running_tasks = [task for task in running_session.plan.tasks if task.status.value == "running"]
    assert len(running_tasks) == 2
    engineer_labels = {task.engineer_label for task in running_tasks}
    assert engineer_labels == {"工程师1号(codex)", "工程师2号(codex)"}
    run_ids = {task.delegate_run_id for task in running_tasks}
    assert len(run_ids) == 2


def test_running_task_without_receipt_triggers_soft_ping_before_hard_intervention(
    ...
    stale_task = running_task.model_copy(
        update={
    ...
    stale_session = session.model_copy(
        update={
            "plan": session.plan.model_copy(
                update={
                    "tasks": [stale_task if task.task_id == stale_task.task_id else task for task in session.plan.tasks]
                }
            ),
            "updated_at": datetime.now(timezone.utc) - timedelta(hours=3),
        }
    )
    orchestrator_service._repository.save(stale_session)

    recovered = orchestrator_service.get_session(session.session_id)
    assert recovered.plan is not None
    soft_ping_task = next(task for task in recovered.plan.tasks if task.task_id == running_task.task_id)
    assert soft_ping_task.status.value == "running"
    assert "进度追问" in (soft_ping_task.result_summary or "")
    assert soft_ping_task.stall_level == "soft_ping"
    assert soft_ping_task.artifacts.get("stall_level") == "soft_ping"
    assert soft_ping_task.artifacts.get("stalled") is False
    assert soft_ping_task.last_stall_followup_at is not None
    assert soft_ping_task.last_intervened_at is not None
    stall_followup = soft_ping_task.artifacts.get("stall_followup")
    assert isinstance(stall_followup, dict)
    assert stall_followup.get("level") == "soft_ping"
    assert isinstance(stall_followup.get("followup_command"), str)
    assert soft_ping_task.stall_followup is not None
    assert soft_ping_task.stall_followup.level == "soft_ping"
    assert len(soft_ping_task.intervention_suggestions) >= 1


def test_apply_directive_can_follow_up_specific_running_engineer(tmp_path: Path) -> None:
    project = tmp_path / "demo-tauri-project"
    (project / "src").mkdir(parents=True)
    (project / "src-tauri" / "src").mkdir(parents=True)
    (project / "src" / "main.ts").write_text("console.log('frontend')\n", encoding="utf-8")
    ...
    service.run_scheduler_tick()
    running_session = service.get_session(session.session_id)
    assert running_session.plan is not None
    running_tasks = [task for task in running_session.plan.tasks if task.status.value == "running"]
    assert len(running_tasks) == 2

    updated = service.apply_directive(running_session.session_id, "追问工程师2号卡点并给建议")
    assert updated.plan is not None
    engineer_two_task = next(
        task for task in updated.plan.tasks if task.artifacts.get("engineer_id") == 2 and task.status.value == "running"
    )
    assert "按指令追问" in (engineer_two_task.result_summary or "")
    assert engineer_two_task.stall_level == "manual_followup"
    assert engineer_two_task.artifacts.get("stall_level") == "manual_followup"
    followup = engineer_two_task.artifacts.get("stall_followup")
    assert isinstance(followup, dict)
    assert followup.get("level") == "manual_followup"
    assert followup.get("followup_command") == "追问工程师2号(codex)卡点并给建议"
    assert engineer_two_task.stall_followup is not None
    assert engineer_two_task.stall_followup.level == "manual_followup"


def test_apply_directive_followup_requires_explicit_engineer_when_multiple_running(tmp_path: Path) -> None:
    project = tmp_path / "demo-tauri-project"
    (project / "src").mkdir(parents=True)
    (project / "src-tauri" / "src").mkdir(parents=True)
    (project / "src" / "main.ts").write_text("console.log('frontend')\n", encoding="utf-8")
    ...
    with pytest.raises(ValueError, match="指定工程师编号"):
        service.apply_directive(session.session_id, "追问工程师卡点并给建议")
...
def test_delete_session_removes_history_and_clears_active_state(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，处理当前项目", str(project_dir))

    removed_messages = orchestrator_service.delete_session(session.session_id)
    assert removed_messages == 0

    with pytest.raises(ValueError, match="not found"):
        orchestrator_service.get_session(session.session_id)

    current_state = orchestrator_service._state_store.get()
    assert current_state.orchestrator_session is None
    assert current_state.focus_mode == FocusMode.AUTONOMY


def test_delete_session_rejects_active_dispatching_session(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，处理当前项目", str(project_dir))
    ...
    with pytest.raises(ValueError, match="active"):
        orchestrator_service.delete_session(session.session_id)
...
def test_apply_directive_can_add_chat_assigned_task(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    ...
    updated = orchestrator_service.apply_directive(
        session.session_id,
        "指派任务：实现主控聊天里直接给 Codex 派任务",
    )

    assert updated.plan is not None
    chat_tasks = [task for task in updated.plan.tasks if task.task_id.startswith("chat-implement-")]
    assert len(chat_tasks) == 1
    assert chat_tasks[0].kind.value == "implement"
    assert "聊天指派" in chat_tasks[0].title
    assert chat_tasks[0].status.value == "pending"
    verify_task = next(task for task in updated.plan.tasks if task.kind.value == "verify")
    assert chat_tasks[0].task_id in verify_task.depends_on
    assert updated.status.value == "dispatching"
    assert updated.coordination is not None
    assert updated.coordination.mode.value == "ready"
    assert "新增聊天任务" in (updated.summary or "")


def test_apply_directive_can_add_chat_task_with_scope_acceptance_and_priority_overrides(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    ...
    updated = orchestrator_service.apply_directive(
        ...
        (
            "指派任务：补齐主控聊天派任务流程；"
            "scope: apps/desktop, services/core；"
            "验收: pytest -q services/core/tests/test_api_orchestrator.py | npm --prefix apps/desktop test -- --run src/pages/OrchestratorPage.test.tsx；"
            "优先级: 最高"
        ),
    )

    assert updated.plan is not None
    chat_task = next(task for task in updated.plan.tasks if task.task_id.startswith("chat-implement-"))
    assert chat_task.scope_paths == ["apps/desktop", "services/core"]
    assert chat_task.acceptance_commands == [
        "pytest -q services/core/tests/test_api_orchestrator.py",
        "npm --prefix apps/desktop test -- --run src/pages/OrchestratorPage.test.tsx",
    ]
    assert chat_task.assignment_source == "chat_assignment"
    assert chat_task.assignment_scope_override == ["apps/desktop", "services/core"]
    assert chat_task.assignment_acceptance_override == [
        "pytest -q services/core/tests/test_api_orchestrator.py",
        "npm --prefix apps/desktop test -- --run src/pages/OrchestratorPage.test.tsx",
    ]
    assert chat_task.assignment_priority_override == 80
    assert chat_task.artifacts.get("scope_override") == ["apps/desktop", "services/core"]
    assert chat_task.artifacts.get("priority_override") == 80

    verify_task = next(task for task in updated.plan.tasks if task.kind.value == "verify")
    assert chat_task.task_id in verify_task.depends_on
    assert updated.priority_bias == 80
    assert updated.summary is not None
    assert "scope=apps/desktop, services/core" in updated.summary
    assert "优先级=最高" in updated.summary


def test_apply_directive_chat_assignment_infers_project_root_scope_from_objective(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    ...
    updated = orchestrator_service.apply_directive(
        session.session_id,
        "指派任务：在当前目录下创建一个 markdown 文件，命名为 test.md",
    )

    assert updated.plan is not None
    chat_task = next(task for task in updated.plan.tasks if task.task_id.startswith("chat-implement-"))
    assert chat_task.scope_paths == ["."]
    assert chat_task.assignment_source == "chat_assignment"
    assert chat_task.assignment_scope_override is None
    assert chat_task.assignment_resolved_scope_override == ["."]
    assert chat_task.artifacts.get("scope_override") is None
    assert chat_task.artifacts.get("resolved_scope_override") == ["."]
    assert updated.summary is not None
    assert "scope=." in updated.summary


def test_complete_delegate_fails_chat_file_creation_task_without_changed_files(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    ...
    orchestrator_service.run_scheduler_tick()
    session = orchestrator_service.get_session(session.session_id)
    assert session.plan is not None
    waiting_reason = (session.coordination.waiting_reason if session.coordination is not None else "") or ""
    if "高风险" in waiting_reason:
        session = orchestrator_service.apply_directive(session.session_id, "批准高风险任务并继续")
        orchestrator_service.run_scheduler_tick()
        session = orchestrator_service.get_session(session.session_id)
        assert session.plan is not None

    running_chat_task = next(
        task
        for task in session.plan.tasks
        if task.status.value == "running" and task.task_id.startswith("chat-implement-")
    )

    failed = orchestrator_service.complete_delegate(
        OrchestratorDelegateCompletionPayload(
            session_id=session.session_id,
            task_id=running_chat_task.task_id,
            delegate_run_id=running_chat_task.delegate_run_id or "",
            result=OrchestratorDelegateResult(
                status="succeeded",
                summary="文件创建完成",
                changed_files=[],
    ...
    assert failed.status.value == "failed"
    assert failed.plan is not None
    failed_chat_task = next(task for task in failed.plan.tasks if task.task_id == running_chat_task.task_id)
    assert failed_chat_task.status.value == "failed"
    assert failed_chat_task.error == "delegate reported success but no changed files for a file-creation task"
...
def test_running_task_without_receipt_triggers_manager_intervention(orchestrator_service: OrchestratorService, project_dir: Path) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    session = orchestrator_service.generate_plan(session.session_id)
    session = orchestrator_service.approve_plan(session.session_id)
    session = orchestrator_service.dispatch(session.session_id)
    ...
    stale_task = running_task.model_copy(
        update={
            "assigned_at": datetime.now(timezone.utc) - timedelta(hours=7),
            "last_stall_followup_at": None,
            "artifacts": {
                **running_task.artifacts,
                "assigned_at": (datetime.now(timezone.utc) - timedelta(hours=7)).isoformat(),
                "last_stall_followup_at": None,
            }
        }
    )
    updated_tasks = [
        stale_task if task.task_id == stale_task.task_id else task
        for task in session.plan.tasks
    ]
    stale_session = session.model_copy(
        update={
    ...
    recovered = orchestrator_service.get_session(session.session_id)
    assert recovered.plan is not None
    stalled_task = next(task for task in recovered.plan.tasks if task.task_id == running_task.task_id)
    assert stalled_task.status.value == "running"
    assert "主控已介入排障" in (stalled_task.result_summary or "")
    stall_followup = stalled_task.artifacts.get("stall_followup")
    assert isinstance(stall_followup, dict)
    assert stall_followup.get("level") == "hard_intervention"
    assert "suggestions" in stall_followup
...
def test_stop_delegate_marks_running_task_failed_and_is_idempotent(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    ...
    assert session.plan is not None
    running_task = next(task for task in session.plan.tasks if task.status.value == "running")
    assert running_task.delegate_run_id is not None

    stopped = orchestrator_service.stop_delegate(
        OrchestratorDelegateStopPayload(
            session_id=session.session_id,
            task_id=running_task.task_id,
            delegate_run_id=running_task.delegate_run_id,
            reason="主控手动停止单任务",
        )
    )

    assert stopped.status.value == "failed"
    assert stopped.plan is not None
    stopped_task = next(task for task in stopped.plan.tasks if task.task_id == running_task.task_id)
    assert stopped_task.status.value == "failed"
    assert stopped_task.error == "主控手动停止单任务"
    assert stopped_task.artifacts.get("stopped_by_orchestrator") is True
    assert stopped_task.last_intervened_at is not None

    idempotent = orchestrator_service.stop_delegate(
        OrchestratorDelegateStopPayload(
            session_id=session.session_id,
            task_id=running_task.task_id,
            delegate_run_id=running_task.delegate_run_id,
    ...
    assert idempotent.session_id == stopped.session_id
    assert idempotent.status.value == "failed"


def test_stop_delegate_rejects_delegate_run_id_mismatch(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    ...
    with pytest.raises(ValueError, match="delegate run id mismatch"):
        orchestrator_service.stop_delegate(
            OrchestratorDelegateStopPayload(
                session_id=session.session_id,
                task_id=running_task.task_id,
                delegate_run_id=f"{running_task.delegate_run_id}-other",
                reason="stop",
            )
        )


def test_list_sessions_supports_status_project_and_keyword_filters(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
    tmp_path: Path,
) -> None:
    ...
    second_project.mkdir()
    (second_project / "package.json").write_text(json.dumps({"name": "second-project"}), encoding="utf-8")
    get_runtime_config().set_folder_permission(str(second_project), "read_only")

    first = orchestrator_service.create_session("进入主控，修复主控工作台", str(project_dir))
    first = orchestrator_service.generate_plan(first.session_id)
    first = orchestrator_service.approve_plan(first.session_id)

    second = orchestrator_service.create_session("进入主控，整理发布脚本", str(second_project))

    by_status = orchestrator_service.list_sessions(statuses=[OrchestratorSessionStatus.DISPATCHING])
    assert {item.session_id for item in by_status} == {first.session_id}

    by_project = orchestrator_service.list_sessions(project="second-project")
    assert {item.session_id for item in by_project} == {second.session_id}

    by_keyword = orchestrator_service.list_sessions(keyword="修复主控工作台")
    assert {item.session_id for item in by_keyword} == {first.session_id}


def test_get_session_normalizes_executor_fields_from_legacy_artifacts(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    ...
    legacy_followup = {
        "level": "soft_ping",
        "elapsed_minutes": 90,
        "manager_summary": "主控先发起进度追问并给出排障建议。",
        "engineer_prompt": "请反馈卡点。",
        "suggestions": ["先给出失败命令和错误摘要。"],
        "followup_command": "追问工程师1号(codex)卡点并给建议",
    }
    legacy_task = running_task.model_copy(
        update={
            "engineer_id": None,
            "engineer_label": None,
            "assigned_at": None,
            ...
            "artifacts": {
                **running_task.artifacts,
                ...
                "assigned_at": (datetime.now(timezone.utc) - timedelta(minutes=90)).isoformat(),
                "stall_level": "soft_ping",
                "last_stall_followup_at": datetime.now(timezone.utc).isoformat(),
                "stall_followup": legacy_followup,
            },
        }
    )
    legacy_session = session.model_copy(
        update={
            "plan": session.plan.model_copy(
                update={
                    "tasks": [
                        legacy_task if task.task_id == legacy_task.task_id else task
                        for task in session.plan.tasks
                    ]
                }
            )
    ...
    orchestrator_service._repository.save(legacy_session)

    normalized = orchestrator_service.get_session(session.session_id)
    assert normalized.plan is not None
    normalized_task = next(task for task in normalized.plan.tasks if task.task_id == running_task.task_id)
    assert normalized_task.engineer_id == 1
    assert normalized_task.engineer_label == "工程师1号(codex)"
    assert normalized_task.assigned_at is not None
    assert normalized_task.stall_level == "soft_ping"
    assert normalized_task.stall_followup is not None
    assert normalized_task.stall_followup.followup_command == "追问工程师1号(codex)卡点并给建议"
    assert normalized_task.last_stall_followup_at is not None
    assert normalized_task.last_intervened_at is not None
    assert normalized_task.intervention_suggestions == ["先给出失败命令和错误摘要。"]


def test_get_session_normalizes_chat_assignment_fields_from_legacy_artifacts(
    orchestrator_service: OrchestratorService,
    project_dir: Path,
) -> None:
    session = orchestrator_service.create_session("进入主控，升级当前项目", str(project_dir))
    ...
    assert session.plan is not None
    chat_task = next(task for task in session.plan.tasks if task.task_id.startswith("chat-implement-"))
    legacy_chat_task = chat_task.model_copy(
        update={
            "assignment_source": None,
            "assignment_directive": None,
            "assignment_requested_objective": None,
            "assignment_scope_override": None,
            "assignment_resolved_scope_override": None,
            "assignment_acceptance_override": None,
            "assignment_priority_override": None,
            "artifacts": {
                **chat_task.artifacts,
                "source": "chat_assignment",
                "directive": "指派任务：补齐主控聊天派任务流程",
                "requested_objective": "补齐主控聊天派任务流程",
                "scope_override": ["apps/desktop", "services/core"],
                "resolved_scope_override": ["apps/desktop", "services/core"],
                "acceptance_override": ["pytest -q services/core/tests/test_api_orchestrator.py"],
    ...
    legacy_session = session.model_copy(
        update={
            "plan": session.plan.model_copy(
                update={
                    "tasks": [
                        legacy_chat_task if task.task_id == legacy_chat_task.task_id else task
                        for task in session.plan.tasks
                    ]
                }
            )
    ...
    normalized = orchestrator_service.get_session(session.session_id)
    assert normalized.plan is not None
    normalized_chat_task = next(task for task in normalized.plan.tasks if task.task_id == chat_task.task_id)
    assert normalized_chat_task.assignment_source == "chat_assignment"
    assert normalized_chat_task.assignment_requested_objective == "补齐主控聊天派任务流程"
    assert normalized_chat_task.assignment_scope_override == ["apps/desktop", "services/core"]
    assert normalized_chat_task.assignment_resolved_scope_override == ["apps/desktop", "services/core"]
    assert normalized_chat_task.assignment_acceptance_override == [
        "pytest -q services/core/tests/test_api_orchestrator.py",
    ]
    assert normalized_chat_task.assignment_priority_override == 80
