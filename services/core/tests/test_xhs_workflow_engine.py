...
def test_build_draft_prompt_omits_empty_persona_fields():
    engine = workflow_engine.XhsWorkDomainEngine(
        state_store=StateStore(),
        memory_repository=InMemoryMemoryRepository(),
    )
...
def test_idle_reviewing_respects_cooldown_before_publishing(monkeypatch):
    engine = workflow_engine.XhsWorkDomainEngine(
        state_store=StateStore(),
        memory_repository=InMemoryMemoryRepository(),
    )
    domain = engine._get_or_init_domain()
    domain.state.status = XhsWorkStatus.IDLE_REVIEWING
    domain.state.idle_reviewing_entered_at = datetime.now(timezone.utc)
    domain.state.pending_drafts = [
        {
            "draft_id": "draft-1",
            "title": "高颜值巧克力怎么发",
    ...
    # Within cooldown: should stay IDLE_REVIEWING and return None
    action = engine.tick()
    updated = engine.state_store.get().xhs_work_domain
    assert action is None
    assert updated.state.status == XhsWorkStatus.IDLE_REVIEWING

    # After cooldown: should proceed to publishing
    domain.state.idle_reviewing_entered_at = datetime.now(timezone.utc) - timedelta(
        seconds=workflow_engine._MIN_IDLE_REVIEWING_SECONDS + 1
    )
    engine._save(domain)
    ...
    action2 = engine.tick()
    updated2 = engine.state_store.get().xhs_work_domain
    assert action2 is not None
    assert action2.kind == "publishing"
    assert updated2.state.status == XhsWorkStatus.IDLE
    assert updated2.state.idle_reviewing_entered_at is None
...
def test_task_chain_is_retryable():
    ...
    assert chain.is_retryable(XhsTaskKind.BLOCKED, "打不开创作首页")
