...
def test_pausing_goal_preserves_remaining_focus_order_instead_of_repository_order():
    repository = InMemoryGoalRepository()
    first_goal = repository.save_goal(
        Goal(title="先暂停当前这条线", created_at=datetime(2026, 4, 5, tzinfo=timezone.utc))
    )
    second_goal = repository.save_goal(
        Goal(title="仓库里更早但当前排在后面的目标", created_at=datetime(2026, 4, 5, 0, 1, tzinfo=timezone.utc))
    )
    third_goal = repository.save_goal(
        Goal(title="当前真正排在下一位的目标", created_at=datetime(2026, 4, 5, 0, 2, tzinfo=timezone.utc))
    )
    state_store = StateStore(
        BeingState(
            mode=WakeMode.AWAKE,
            focus_mode=FocusMode.MORNING_PLAN,
            active_goal_ids=[first_goal.id, third_goal.id, second_goal.id],
            today_plan={
                "goal_id": first_goal.id,
                "goal_title": first_goal.title,
                "steps": [{"content": f"把“{first_goal.title}”的轮廓理一下", "status": "pending"}],
    ...
    try:
        ...
        assert response.status_code == 200
        current_state = state_store.get()
        assert current_state.active_goal_ids == [third_goal.id, second_goal.id]
        assert current_state.today_plan is not None
        assert current_state.today_plan.goal_id == third_goal.id
        assert current_state.today_plan.goal_title == third_goal.title
    finally:
        app.dependency_overrides.clear()
