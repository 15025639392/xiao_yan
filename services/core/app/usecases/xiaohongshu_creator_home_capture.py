...
def capture_xiaohongshu_creator_home_from_chrome() -> XiaohongshuCreatorHomeCaptureResponse:
    captured = capture_active_chrome_tab()
    if not captured.url.startswith(_EXPECTED_URL_PREFIX):
        raise ValueError("active Chrome tab is not xiaohongshu creator home")

    extracted = extract_creator_home_from_raw_text(captured.body_text)
