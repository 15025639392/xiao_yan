...
DEFAULT_TOOL_REGISTRY: dict[str, ToolMetadata] = {
    ...
    "brew": ToolMetadata(
        name="brew",
        description="Homebrew 环境补全工具（执行前需要能力审批）",
        safety_level=ToolSafetyLevel.RESTRICTED,
        category="dev",
        examples=["brew install --cask blender", "brew --version"],
        max_timeout_seconds=120.0,
    ),
    "curl": ToolMetadata(
        name="curl",
        description="下载外部安装资源（执行前需要能力审批）",
        safety_level=ToolSafetyLevel.RESTRICTED,
        category="network",
        examples=["curl -fsSL https://example.com/file -o /tmp/file"],
        max_timeout_seconds=120.0,
    ),
    "bash": ToolMetadata(
        name="bash",
        description="执行本地 shell 脚本（执行前需要能力审批）",
        safety_level=ToolSafetyLevel.RESTRICTED,
        category="dev",
        examples=["bash /tmp/install.sh"],
        max_timeout_seconds=120.0,
    ),
    "node": ToolMetadata(
        name="node",
